...
void RNAData::initialize(DimerLibArray &L, int idx, int idx_L, gsl_block *MemBlock, size_t *offset_matrix, uint16_t *ArrayMemBlock, size_t *offset_array)
{
	...
	position_in_lib.idx1 = idx;
	position_in_lib.idx2 = idx_L;
	ResBoundaries[0] = 0;
	...
	*offset_matrix += L[idx]->data_matrices[idx_L]->size1 * L[idx]->data_matrices[idx_L]->size2;
	...
	count_per_sub.idx1 = get_target(name[0], &target1);

	submatrix_rows[0] = &ArrayMemBlock[*offset_array];
	*offset_array += count_per_sub.idx1;

	count_per_sub.idx2 = get_target(name[1], &target2);

	submatrix_rows[1] = &ArrayMemBlock[*offset_array];
	*offset_array += count_per_sub.idx2;

	for (uint j = 0; j < count; j++) {
		for (uint k = 0; k < count_per_sub.idx1; k++) {
			if ((target1[k] == atom_data->atom_ids[j]) && (atom_data->dnt_pos[j] - 1) == 0) {
				submatrix_rows[0][k] = j;
				rel_idx1++;
			}
		}
		for(uint k = 0; k < count_per_sub.idx2; k++) {
	...
	*offset_array += count_per_WC_sub[1];
	...
	count_per_Steric.idx1 = rel_idx1;
	*offset_array += rel_idx1;
	...
	count_per_Steric.idx2 = rel_idx2;
...
void RNAData::overwrite(DimerLibArray &L, int i, int j)
{
	...
	energy = L[i]->energy[j];
	_flag = &(L.Flags[i][j]);
	position_in_lib.idx1 = i;
	position_in_lib.idx2 = j;    
}
...
{
	for (uint i = 0; i < count_per_sub[res]; i++)
	{
		atom_data->print_at(submatrix_rows[res][i]);
		for (uint j = 0; j < data_matrix->size2; j++)
		{
...
int RNAData::overwrite_string_prototype(int res, int position, char *s, int buffer_size, int string_index, uint32_t& idx_offset)
{
...
void RNADataArray::initialize_ds(RunInfo& run_info, DimerLibArray& wc_library, size_t& arr_offset)
{
	...
	for(int i = 0; i < wc_library[0]->atom_data->count; i++)
	{
		for(size_t j = 0; j < ds_data.wc_rowsize[0]; j++) 
		{
			if(wc_library[0]->atom_data->atom_ids[i] == wc_target1[j] && wc_library[0]->atom_data->dnt_pos[i] == 1)
			{
		...
		{
			if(wc_library[0]->atom_data->atom_ids[i] == wc_target2[j] && wc_library[0]->atom_data->dnt_pos[i] == 2)
			{
...
void RNADataArray::initialize(RunInfo& run_info, DimerLibArray& library, DimerLibArray& wc_library)
{
	...
	if(run_info.structure_type == StrType::double_strand) {
		...
		ds_data.wc_rowsize[0] = get_target(wc_library[0]->name[0], &wc_target1);
		ds_data.wc_rowsize[1] = get_target(wc_library[0]->name[1], &wc_target2);
		array_memsize += sizeof(uint16_t) * (ds_data.wc_rowsize[0] + ds_data.wc_rowsize[1]); // Only needed for Double Strand.
	...
	*sequence[0]._flag = USED;
	...
	if(n_wc_pairs != 0)
	{
		...
		for(uint i = 0; i < n_wc_pairs; i++) {
			...
			if(structure_type == StrType::double_strand) {
				if(idx1 < (uint)(ds_data.strand1_size + 1)) {
					wc_pairs[i].idx1 = idx1 < 2 ? 0 : idx1 - 1;
					wc_pair_res[i].idx1 = idx1 == 0 ? 0 : 1;
				} else {
					wc_pairs[i].idx1 = idx1 < (uint)(ds_data.strand1_size + 4) ? ds_data.strand1_size : idx1 - 3;
					wc_pair_res[i].idx1 = idx1 == (uint)(ds_data.strand1_size + 2) ? 0 : 1;
				}
				if(idx2 < (uint)(ds_data.strand1_size + 1)) {
					wc_pairs[i].idx2 = idx2 < 2 ? 0 : idx2 - 1;
					wc_pair_res[i].idx2 = idx2 == 0 ? 0 : 1;
				} else {
					wc_pairs[i].idx2 = idx2 < (uint)(ds_data.strand1_size + 4) ? ds_data.strand1_size : idx2 - 3;
					wc_pair_res[i].idx2 = idx2 == (uint)(ds_data.strand1_size + 2) ? 0 : 1;
				}
			} else {
				wc_pairs[i].idx1 = idx1 < 2 ? 0 : idx1 - 1;
				wc_pair_res[i].idx1 = idx1 == 0 ? 0 : 1;
			...
			//DEBUG_PRINT("INITIALIZATION: idx 1: %lu, 2: %lu\n", idx1, idx2);
			//DEBUG_PRINT("INITIALIZATION: wc_pairs[%u] 1: %lu, 2: %lu\n", i, wc_pairs[i].idx1, wc_pairs[i].idx2);
			//DEBUG_PRINT("INITIALIZATION: wc_pair_res[%u] 1: %lu, 2: %lu\n", i, wc_pair_res[i].idx1, wc_pair_res[i].idx2);
			wc_rmsd[i] = 0.00;
		}
	}
...
void RNADataArray::update_energy()
{
	...
	} else if (structure_type == StrType::double_strand){	
		//Find interactions within each strand
		//Left Strand
		for(int i = ds_data.strand1_size - 1; i > 0; i--) {
			Interactions += FindInteraction(0, 0, &sequence[0], 1, i, &sequence[i],
					InteractionTable, PositiveAtomMap, NegativeAtomMap, LAC);
		}
		for(int i = ds_data.strand1_size; i > 1; i--) {
			Interactions += FindInteraction(1, 0, &sequence[0], 1, i, &sequence[i],
					InteractionTable, PositiveAtomMap, NegativeAtomMap, LAC);
		}
		for (int i = 1; i <= ds_data.strand1_size - 3; i++) {
			for (int j = ds_data.strand1_size - 1; j > i + 1; j--) {
				Interactions += FindInteraction(1, i, &sequence[i], 1, j, &sequence[j],
						InteractionTable, PositiveAtomMap, NegativeAtomMap, LAC);
			}
		}
		//Right Strand	
		for(int i = count - 1; i > ds_data.strand1_size; i--) {
			Interactions += FindInteraction(0, ds_data.strand1_size, &sequence[0], 1, i, &sequence[i],
					InteractionTable, PositiveAtomMap, NegativeAtomMap, LAC);
		}
		for(int i = count - 1; i > ds_data.strand1_size + 1; i--) {
			Interactions += FindInteraction(1, ds_data.strand1_size, &sequence[0], 1, i, &sequence[i],
					InteractionTable, PositiveAtomMap, NegativeAtomMap, LAC);
		}
		for (int i = ds_data.strand1_size + 1; i <= count - 3; i++) {
			for (int j = count - 1; j > i + 1; j--) {
				Interactions += FindInteraction(1, i, &sequence[i], 1, j, &sequence[j],
						InteractionTable, PositiveAtomMap, NegativeAtomMap, LAC);
			}
		}
		//Find interactions across strands
		Interactions += FindInteraction(0, 0, &sequence[0], 0, ds_data.strand1_size, &sequence[ds_data.strand1_size],
				InteractionTable, PositiveAtomMap, NegativeAtomMap, LAC);
		for(int i = ds_data.strand1_size; i < count; i++) {
			Interactions += FindInteraction(0, 0, &sequence[0], 1, i, &sequence[i],
					InteractionTable, PositiveAtomMap, NegativeAtomMap, LAC);
		}
		for(int i = 0; i < ds_data.strand1_size; i++) {
			Interactions += FindInteraction(1, i, &sequence[i], 0, ds_data.strand1_size, &sequence[ds_data.strand1_size],
					InteractionTable, PositiveAtomMap, NegativeAtomMap, LAC);
			for(int j = ds_data.strand1_size; j < count; j++) {
				Interactions += FindInteraction(1, i, &sequence[i], 1, j, &sequence[j],
						InteractionTable, PositiveAtomMap, NegativeAtomMap, LAC);
			}
		}
...
int RNADataArray::out_string_header_coord(RunInfo& run_info)
{
	...
	for (int i = 0; i < count; i++) {
		string_index += snprintf(&string_out[string_index], string_buffer - string_index, "%lu", sequence[i].position_in_lib.idx2  + 1);
		if (i != count - 1) {
...
void RNADataArray::overwrite_string_prototype_ds(uint32_t& idx_offset)
{
	...
	for(int i = 1; i <= iterator_max; i++, pos++) {
		if(i == ds_data.iterator_max_Strand1 + 1) {
			string_index += snprintf(&string_out[string_index], string_buffer - string_index, "TER\n");
			pos += 1;
			string_index = sequence[i].overwrite_string_prototype(0, pos, string_out, string_buffer, string_index, idx_offset);
			string_index = sequence[i].overwrite_string_prototype(1, pos, string_out, string_buffer, string_index, idx_offset);
		} else {
			string_index = sequence[i].overwrite_string_prototype(1, pos, string_out, string_buffer, string_index, idx_offset);
		}
	}
...
{
	for(int i = 1; i <= iterator_max; i++) {
		string_index = sequence[i].overwrite_string_prototype(1, i, string_out, string_buffer, string_index, idx_offset);
	}
	string_index = snprintf(&string_out[string_index], string_buffer - string_index, "ENDMDL\n");
}
...
char* RNADataArray::overwrite_string_prototype(RunInfo& run_info)
{
	...
	string_index = out_string_header(run_info);
	if(run_info.run_options & RunOpts::write_coordinates) {
		//snprintf will always attach a '\0' to the end of the string it creates. So we replace '\0' with 'A' b/c next line starts w/ "ATOM"
		//string_out[string_index] = 'A'; 
		string_index = sequence[0].overwrite_string_prototype(0, 0, string_out, string_buffer, string_index, idx_offset);
		string_index = sequence[0].overwrite_string_prototype(1, 0, string_out, string_buffer, string_index, idx_offset);
		if(structure_type == StrType::double_strand) {
...
int* RNADataArray::get_index()
{
	...
	for (int i = 0; i < count; i++) {
		ar[i] = sequence[i].position_in_lib.idx2;
	}
	return ar; 
}
...
void RNADataArray::print_index(int offset)
{
	for (int i = 0; i < count - 1 + offset; i++) {
		printf("%lu-", sequence[i].position_in_lib.idx2);
	}
	printf("%lu\n", sequence[count - 1 + offset].position_in_lib.idx2);
}
...
void RNADataArray::print_index()
{
	...
	printf("%lu\n", sequence[count - 1].position_in_lib.idx2);
}
