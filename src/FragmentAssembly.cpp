...
AttachStatus attach_5p_to_3p(RNAData *reference, RNAData *rotated, RunInfo& run_info)
{
	...
	R = kabsch_calculate_rotation_matrix_Nx3fast(P, Q, P_WORK, COMP, COMQ);
	rmsd = rmsd_generic(P, Q);

	if (rmsd <= run_info.rmsd_limit) {
		status = ATTACHED;
...
template <uint32_t OPTS> AttachStatus prepare_right(RNAData* to_be_assembled, DimerLibArray &WC_Lib, RNADataArray &assembled, RunInfo& run_info) 
{
	...
	gsl_matrix *WC_MODEL = WC_Lib[0]->data_matrices[0];
	...
	R1 = kabsch_calculate_rotation_matrix_Nx3fast(P1, Q1, P1WORK, COMP, COMQ);
	...
	R2 = kabsch_calculate_rotation_matrix_Nx3fast(P2, Q2, P2WORK, COMP, COMQ);
...
AttachStatus fragment_assembly_ss(DimerLibArray &Lib, RNADataArray &assembled, CMB_Manager &manager, RunInfo& run_info)
{
	...
	for (int i = library_position; i < manager.lib_bounds[dnmp_position].idx2; i++) {
		...
		if (attach_5p_to_3p(assembled.current(), assembled[dnmp_position], run_info) != AttachStatus::ATTACHED) {
			continue;
		}
...
template <uint32_t OPTS> AttachStatus fragment_assembly_ds(DimerLibArray &Lib, DimerLibArray &WC_Lib, RNADataArray &assembled, CMB_Manager &manager, RunInfo& run_info)
{
	uint32_t dnmp_position = assembled.iterator + 1; // Position in sequence where new DNMP will be attached.
	...
	if (assembled.should_prepare_right(dnmp_position) == true) {
		for (int i = library_position; i < manager.lib_bounds[dnmp_position].idx2; i++) {
			assembled.overwrite(dnmp_position, i, Lib);
			manager.attach_attempt(dnmp_position, i);
			if (prepare_right<OPTS>(assembled[dnmp_position], WC_Lib, assembled, run_info) == AttachStatus::ATTACHED) {
				return AttachStatus::ATTACHED;
			}
			continue;
		}
...
template <uint32_t OPTS> AttachStatus fragment_assembly(DimerLibArray &lib, DimerLibArray &wc_lib, RNADataArray &assembled, CMB_Manager &manager, RunInfo& run_info)
{
	if(OPTS & RunOpts::strtype_ds) {
		if(OPTS & RunOpts::str_filter_uses_ds_closing_bp) {
			return fragment_assembly_ds<RunOpts::str_filter_uses_ds_closing_bp>(lib, wc_lib, assembled, manager, run_info);
		} else {
			return fragment_assembly_ds<0>(lib, wc_lib, assembled, manager, run_info);
		}
	} else {
		return fragment_assembly_ss(lib, assembled, manager, run_info);
	}
}
template AttachStatus fragment_assembly<RunOpts::strtype_ds | RunOpts::str_filter_uses_ds_closing_bp>(DimerLibArray &lib, DimerLibArray &wc_lib, RNADataArray &assembled, CMB_Manager &manager, RunInfo& run_info);
template AttachStatus fragment_assembly<RunOpts::strtype_ds>(DimerLibArray &lib, DimerLibArray &wc_lib, RNADataArray &assembled, CMB_Manager &manager, RunInfo& run_info);
template AttachStatus fragment_assembly<0>(DimerLibArray &lib, DimerLibArray &wc_lib, RNADataArray &assembled, CMB_Manager &manager, RunInfo& run_info);
